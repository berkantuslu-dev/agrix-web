...
if (isset($_POST["loggin"])) {
    header("Location: logintz.php"); // Redirect to login page
...
if (isset($_POST["signup"])) {
    ...
    if (empty($_POST["username"])) {
        ...
        $username_err = "Your username should contain more than 3 characters";
    } elseif (!preg_match('/^[a-zA-Z0-9_]+$/', $_POST["username"])) {
        $username_err = "Your username can not contain special characters";
    } else {
    ...
    if (empty($_POST["email"])) {
        ...
        if (empty($_POST["phone"])) {
            $phone_err = "Simu haiwezi kuwa tupu.";
        } elseif (!preg_match('/^[0-9]{10,15}$/', $_POST["phone"])) {
            $phone_err = "Nambari ya simu inapaswa kuwa na tarakimu tu na iwe na urefu wa kati ya herufi 10 hadi 15.";
        } else {
            ...
            if (mysqli_num_rows($check_phone_result) > 0) {
                $phone_err = "Nambari hii ya simu tayari imejiandikisha.";
            }
        }
    ...
    if (empty($_POST["password-rpt"])) {
        ...
        $passwordrpt_err = "Nywila hazifanani";
    }
    ...
    if (empty($username_err) && empty($email_err) && empty($password_err) && empty($passwordrpt_err)) {
        ...
        if ($calistirekle) {
            ...
            $error_message = '<div style="background-color: #f44336; color: white; padding: 10px; border-radius: 5px; margin-top: 10px;">
                                Usajili umeshindikana kutokana na kosa la database.
                            </div>';
                            echo $error_message;
        }
...
<head>
    ...
    <title>Portal ya Agrix</title>
	<meta name="description" content="Agrix ni kampuni ya teknolojia ya kilimo ya kisasa iliyoko Dar es Salaam, Tanzania. Tunawapa nguvu wakulima na wauzaji kupitia teknolojia za kilimo za kisasa, ufikiaji wa masoko, na programu za elimu ili kuboresha uzalishaji, faida, na uendelevu katika sekta ya kilimo.">
<meta name="keywords" content="Tanzania agritech, kilimo cha kisasa, kilimo endelevu, mipango ya msaada kwa wakulima, elimu ya kilimo, ufikiaji wa masoko, kilimo cha Dar es Salaam, uvumbuzi wa kilimo, suluhisho za wakulima, uchambuzi wa kilimo, maendeleo ya biashara za kilimo.">
...
<div class="container p-5">
    <div class="card p-5">
        <form action="signin.php" method="POST">
            ...
            <h1>JISAJILI</h1>
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Kijitambulisho</label>
    ...
    <div class="invalid-feedback">
        <?php echo $phone_err; ?>
            ...
            <button type="submit" name="loggin" class="btn btn-primary">Ingiza</button>
        </form>
    </div>
</div>
</body>
